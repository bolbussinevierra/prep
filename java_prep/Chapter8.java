...
public class Chapter8 {
  public static void main(String[] args) {
    ...
    List.of(3, 1, 3, 2, 0).forEach(queueWithMax::enqueue);
    queueWithMax.display();
    queueWithMax.enqueue(1);
    queueWithMax.display();
    queueWithMax.dequeue(); // -3
    queueWithMax.display();
    queueWithMax.dequeue(); // -1
    queueWithMax.display();
  ...
  private static class Stack {
    ...
    public void push(Integer i) {
      elements.addFirst(new ElementWithCachedMax(i, Math.max(i, empty() ? i : max())));
    }
  }
  ...
  // 8.9: Queue with Max
  private static class QueueWithMax<T extends Comparable<T>> {
    private final java.util.Queue<T> q = new ArrayDeque<>();
    private final Deque<T> candidatesForMax = new ArrayDeque<>();

    public void enqueue(T o) {
      q.add(o);
      while (!candidatesForMax.isEmpty() && candidatesForMax.peekLast().compareTo(o) < 0) {
        candidatesForMax.removeLast();
      }
      candidatesForMax.addLast(o);
    }

    public T dequeue() {
      T o = q.remove();
      if (candidatesForMax.peekFirst().equals(o)) {
    ...
    public void display() {
      ...
      System.out.println("D: " + candidatesForMax);
    }
